...
class UserViewSet(UserViewSet):
    ...
    def subscribe(self, request, id=None):
        ...
        if self.request.method == 'POST':
            ...
            if sub.exists():
                ...
                serializer = SubscriptionsSerializer(Subscriptions.objects.create(user=user, author=author),
        ...
        if Subscriptions.objects.filter(user=user, author=author).exists():
            subscribe = get_object_or_404(SubscriptionsSerializer, user=user, author=author)
            subscribe.delete()
    ...
    def subscriptions(self, request):
        user = request.user
        queryset = self.paginate_queryset(Subscriptions.objects.filter(user=user))
